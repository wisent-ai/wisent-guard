...
from .probe_optimization import ProbeOptimizer, ProbeConfig
from .steering_optimization import SteeringOptimizer, DACConfig
...
class ComprehensiveEvaluationPipeline:
    ...
    def _run_probe_optimization(self) -> Dict[str, Any]:
        ...
        probe_config = ProbeConfig(
            ...
            c_values=self.config.probe_c_values
        )
        ...
        probe_training_results = self.probe_optimizer.train_probes(
            self.model, self.tokenizer, self.train_samples, 
            self.config.device, self.config.batch_size, self.config.max_length, self.config.train_dataset, self.config.max_new_tokens
        )
        ...
        best_probe_config, validation_metrics = self.probe_optimizer.evaluate_probes(
            probe_training_results, self.model, self.tokenizer, self.val_samples,
            self.config.device, self.config.batch_size, self.config.max_length, self.config.val_dataset, self.config.max_new_tokens
        )
        ...
        self.logger.info(f"✅ Probe optimization completed. Best: Layer {best_probe_config['layer']}, AUC {best_probe_config['metrics']['auc']:.3f}")
        
        # Log to wandb
        if self.config.enable_wandb:
            wandb.log({
                "probe_best_layer": best_probe_config['layer'],
                "probe_best_auc": best_probe_config['metrics']['auc']
            })
    ...
    def _run_steering_optimization(self) -> Dict[str, Any]:
        ...
        for config in steering_configs:
            ...
            best_config, method_results = self.steering_optimizer.optimize_steering_hyperparameters(
                config, self.train_samples, self.val_samples,
                self.model, self.tokenizer, self.config.device,
                self.config.batch_size, self.config.max_length, self.config.train_dataset, self.config.max_new_tokens
            )
    ...
    def _run_final_evaluation(self, probe_results: Dict[str, Any], steering_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        base_predictions, ground_truths = data_utils.generate_benchmark_predictions(
            self.model, self.tokenizer, self.test_samples,
            self.config.batch_size, self.config.max_length, self.device, self.config.test_dataset, self.config.max_new_tokens
        )
        base_benchmark_metrics = metrics.evaluate_benchmark_performance(base_predictions, ground_truths, self.config.test_dataset)
        ...
        if best_steering_config and best_steering_config.get("method_instance"):
            ...
            steered_predictions, _ = trainer.apply_steering_and_evaluate(
                method_instance, self.test_samples, 
                best_steering_config["layer"], best_steering_config["strength"],
                self.model, self.tokenizer, self.device,
                self.config.batch_size, self.config.max_length, self.config.test_dataset, self.config.max_new_tokens
            )
            steered_benchmark_metrics = metrics.evaluate_benchmark_performance(steered_predictions, ground_truths, self.config.test_dataset)
    ...
    def _create_probe_data_from_predictions(self, test_samples: List[Dict], predictions: List[str], ground_truths: List[str], layer: int) -> Tuple[np.ndarray, np.ndarray]:
        ...
        activations = data_utils.extract_activations_with_hook(
            self.model, self.tokenizer, texts, layer, 
            self.config.batch_size, self.config.max_length, self.device
        )
        
        return activations, np.array(labels)
    
    def _evaluate_probe_on_test(self, probe_config: Dict[str, Any], steered: bool = False, steering_config: Dict[str, Any] = None, stored_data: Dict[str, Any] = None) -> Dict[str, float]:
        ...
        if stored_data:
            ...
            X_test, y_test = self._create_probe_data_from_predictions(
                test_samples, predictions, ground_truths, layer
            )
        else:
            ...
            X_test, y_test = data_utils.create_probe_training_data(
                self.model, self.tokenizer, self.test_samples, layer,
                self.config.batch_size, self.config.max_length, self.device, self.config.test_dataset, self.config.max_new_tokens
            )
    ...
    def _collect_final_results(self, probe_results: Dict[str, Any], steering_results: Dict[str, Any], test_results: Dict[str, Any]) -> Dict[str, Any]:
