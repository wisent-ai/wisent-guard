...
class DACTrainer(SteeringMethodTrainer):
    ...
    def apply_steering_and_evaluate(
        ...
        if hasattr(model, 'model') and hasattr(model.model, 'layers'):
            ...
            raise ValueError(f"Unsupported model architecture for DAC steering")
    ...
    def _create_pair_set_from_extracted_pairs(self, extracted_pairs: List[Dict], layer_index: int, model, tokenizer, device: str) -> ContrastivePairSet:
    ...
    def _extract_activations_for_text(self, text: str, layer_index: int, model, tokenizer, device: str) -> torch.Tensor:
        ...
        if hasattr(model, 'model') and hasattr(model.model, 'layers'):
            ...
            raise ValueError(f"Unsupported model architecture for activation extraction")
...
class SteeringOptimizer:
    ...
    def optimize_steering_hyperparameters(
        ...
        self.logger.info(
            f"Starting {method_name} optimization with {len(hyperparameter_combinations)} configurations"
        )
        ...
        for i, (layer, strength, hyperparams) in enumerate(tqdm(hyperparameter_combinations, desc="Optimizing steering hyperparameters")):
            self.logger.debug(
            ...
            try:
                ...
                self.logger.debug(
                    f"Config {i+1} - Benchmark accuracy: {score:.3f}"
                )
    ...
    def _generate_hyperparameter_combinations(self, config: SteeringMethodConfig) -> List[Tuple[int, float, Dict[str, Any]]]:
