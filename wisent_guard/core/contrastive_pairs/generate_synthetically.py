...
from .generation_resources import (
    SCENARIO_GEN, SCENARIO_PARSE, CONTRASTIVE_GEN
)

class SyntheticContrastivePairGenerator:
    """Generate contrastive pairs synthetically from natural language trait descriptions."""

    def __init__(self, model: Model, similarity_threshold: float = 0.8, db_path: Optional[str] = None, db_similarity_threshold: float = 0.95):
        """
        ...
        self.similarity_model: Optional[SentenceTransformer] = self._initialize_similarity_model()
        self.database: Optional[ContrastivePairDatabase] = self._initialize_database(db_path)
    ...
    def _initialize_similarity_model(self) -> Optional[SentenceTransformer]:
        ...
        try:
            ...
            print(f"⚠️ WARNING: Could not load similarity model ('all-MiniLM-L6-v2'). VDB caching and diversity selection will be disabled. Error: {e}")
            return None

    def _initialize_database(self, db_path: Optional[str]) -> Optional[ContrastivePairDatabase]:
        ...
        try:
            ...
            print(f"🗂️  Initialized contrastive pair database at: {Path(db_path).resolve()}")
            return database
        except (IOError, OSError, json.JSONDecodeError, RuntimeError) as e:
            print(f"⚠️ WARNING: Failed to initialize contrastive pair database at '{db_path}'. Caching is disabled. Error: {e}")
            return None
    ...
    def generate_scenarios(self, trait_description: str, num_scenarios: int) -> list[str]:
        ...
        num_prompts_per_template: int = target_scenarios // len(SCENARIO_GEN.PROMPT_TEMPLATES)

        for i, template in enumerate(SCENARIO_GEN.PROMPT_TEMPLATES):
            prompt: str = template.format(num_prompts=num_prompts_per_template)
            print(f"🎯 DEBUG: Using prompt template {i+1}/{len(SCENARIO_GEN.PROMPT_TEMPLATES)}")
        ...
        selected_scenarios: list[str] = self._select_diverse_scenarios(unique_scenarios, num_scenarios)
    ...
    def _parse_scenarios_from_response(self, response: str) -> list[str]:
        ...
        for line in lines:
            ...
            # 2. Basic filtering
            if not cleaned or not (SCENARIO_PARSE.MIN_SCENARIO_LENGTH < len(cleaned) < SCENARIO_PARSE.MAX_SCENARIO_LENGTH):
                continue

            cleaned_lower: str = cleaned.lower()

            # 3. Filter based on content
            if any(phrase in cleaned_lower for phrase in SCENARIO_PARSE.SKIP_PHRASES) or \
               any(phrase in cleaned_lower for phrase in SCENARIO_PARSE.REFUSAL_PHRASES):
                continue

            # 4. Validate based on structure (question or imperative)
            word_count: int = len(cleaned.split())
            is_question: bool = '?' in cleaned and word_count <= SCENARIO_PARSE.MAX_QUESTION_WORDS
            is_imperative: bool = any(kw in cleaned_lower for kw in SCENARIO_PARSE.IMPERATIVE_KEYWORDS) and word_count <= SCENARIO_PARSE.MAX_IMPERATIVE_WORDS
    ...
    def _deduplicate_scenarios(self, scenarios: list[str]) -> list[str]:
        ...
        for scenario in scenarios:
            ...
            if unique_scenarios:
                ...
                scenario_embedding: np.ndarray = self.similarity_model.encode([scenario])
                existing_embeddings: np.ndarray = self.similarity_model.encode(unique_scenarios)

                # Calculate cosine similarities
                similarities: np.ndarray = np.dot(scenario_embedding, existing_embeddings.T)[0]
    ...
    def _select_diverse_scenarios(self, scenarios: list[str], target_count: int) -> list[str]:
        ...
        for _ in range(target_count - 1):
            remaining_indices: list[int] = [i for i in range(len(scenarios)) if i not in selected_indices]
            ...
            for idx in remaining_indices:
                ...
                for selected_idx in selected_indices:
                    distance: float = 1 - np.dot(embeddings[idx], embeddings[selected_idx])
    ...
    def _get_pair_embedding(self, pair: ContrastivePair) -> np.ndarray:
        ...
        pos_emb: np.ndarray = self.similarity_model.encode([pair.positive_response.text])[0]
        neg_emb: np.ndarray = self.similarity_model.encode([pair.negative_response.text])[0]
    ...
    def _select_diverse_pairs(self, pairs: list[ContrastivePair], target_count: int) -> list[ContrastivePair]:
        ...
        print(f"🔎 Selecting {target_count} diverse pairs from {len(pairs)} candidates...")

        embeddings: np.ndarray = np.array([self._get_pair_embedding(p) for p in pairs])

        selected_indices: list[int] = [0]
        for _ in range(target_count - 1):
            remaining_indices: list[int] = [i for i in range(len(pairs)) if i not in selected_indices]
            if not remaining_indices:
                break
            ...
            dist_matrix: np.ndarray = 1 - np.dot(remaining_embeddings, selected_embeddings.T)
    ...
    def generate_contrastive_pair(self, scenario: str, trait_description: str) -> ContrastivePair:
        ...
        positive_response: str = self._generate_response(positive_prompt, CONTRASTIVE_GEN.CONFIG)
        ...
        negative_response: str = self._generate_response(negative_prompt, CONTRASTIVE_GEN.CONFIG)
    ...
    def generate_contrastive_pair_set(
        ...
        if use_database:
            ...
            trait_embedding: np.ndarray = self.similarity_model.encode([trait_description])

            found_set_info: Optional[dict] = self.database.search_for_trait(trait_embedding, self.db_similarity_threshold)

            if found_set_info is not None:
                cached_pair_count: int = found_set_info.get("pair_count", 0)
                print(f"✅ Found sufficiently similar pair set in cache (ID: {found_set_info['set_id']}) with {cached_pair_count} pairs.")
                ...
                if num_pairs <= cached_pair_count:
                    ...
                    print(f"⚠️ Cache has {cached_pair_count} pairs, but {num_pairs} are requested. Augmenting the set...")
                    # Load existing set and generate the missing pairs
                    existing_set: ContrastivePairSet = self.database.get_set_by_id(found_set_info['set_id'])
                    ...
                    print(f"🔄 Generating {additional_pairs_needed} additional pairs...")
        ...
        print(f"ℹ️ No suitable pair set found in cache or caching is disabled. Generating a new one for trait: '{trait_description}'")
        ...
        new_pair_set = self._generate_new_pairs(trait_description, num_pairs, pair_overgeneration_factor)
    ...
    def _generate_new_pairs(
        ...
        trait_description: str, 
        num_pairs: int, 
        pair_overgeneration_factor: float
    ) -> ContrastivePairSet:
        ...
        scenarios: list[str] = self.generate_scenarios(trait_description, num_scenarios_to_generate)
        ...
        for i, scenario in enumerate(scenarios):
            try:
                ...
                pair: ContrastivePair = self.generate_contrastive_pair(scenario, trait_description)
        ...
        diverse_pairs: list[ContrastivePair] = self._select_diverse_pairs(all_pairs, num_pairs)
        ...
        return quality_check_synthetic_pairs(pair_set, trait_description, strict_mode=True)
    ...
    def _combine_and_update_set(
        ...
        combined_pairs_map: dict[str, ContrastivePair] = {p.prompt: p for p in existing_set.pairs}
        ...
        final_diverse_pairs: list[ContrastivePair] = self._select_diverse_pairs(all_unique_pairs, target_pair_count)
        ...
        print(f"💾 Caching augmented pair set with {len(final_diverse_pairs)} pairs to database...")
...
def generate_synthetic_pairs_cli(
    ...
    generator: SyntheticContrastivePairGenerator = SyntheticContrastivePairGenerator(model)
