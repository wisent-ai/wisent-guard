...
class OptunaBigCodeEvaluator:
        ...
        for i, (prediction, task_doc) in enumerate(zip(predictions, task_docs)):
            try:
                ...
                if not extracted_code.strip():
                    ...
                    result = self.bigcode_evaluator._execute_and_test(
                        task_doc, extracted_code, task_name
                    )
    ...
    def calculate_accuracy(self, evaluation_results: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        fail_count = sum(1 for result in evaluation_results if result.get('passed', False) == False and not result.get('error'))
