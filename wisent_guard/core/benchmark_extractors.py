...
class WinograndeExtractor(BenchmarkExtractor):
    ...
    def extract_qa_pair(self, doc: Dict[str, Any], task_data: Any = None) -> Optional[Dict[str, str]]:
        ...
        try:
            ...
            # Create the question
            formatted_question = f"Complete the sentence: {sentence}"
...
class SwagExtractor(BenchmarkExtractor):
    ...
    def extract_qa_pair(self, doc: Dict[str, Any], task_data: Any = None) -> Optional[Dict[str, str]]:
        ...
        doc['startphrase']: context
        doc[ending1], doc['ending2'], doc['ending3'], doc['ending4] possible endings
        doc[label] index of correct ending

        """
        try:
            
            startphrase = doc.get("startphrase", "")
            endings = [doc.get("ending0", ""), doc.get("ending1", ""), doc.get("ending2", ""), doc.get("ending3", "")]
            label = doc.get("label", 0)

            if not all([startphrase, endings]) or label >= len(endings):
                return None
            ...
            if hasattr(task_data, "doc_to_text"):
                ...
                formatted_question = f"Context: {startphrase}\nChoose the best ending:"

            # Get correct answer
            correct_answer = endings[label]

            return {
                "question": startphrase,                  #f"Context: {ctx}\nChoose the best ending:"
                "formatted_question": formatted_question,
                "correct_answer": correct_answer,
            }
    ...
    def extract_contrastive_pair(self, doc: Dict[str, Any], task_data: Any = None) -> Optional[Dict[str, str]]:
        """Extract contrastive pair for Swag."""
        try:
            qa_pair = self.extract_qa_pair(doc, task_data)
            if not qa_pair:
                return None
...
class TruthfulQAExtractor(BenchmarkExtractor):
    ...
    def extract_contrastive_pair(self, doc: Dict[str, Any], task_data: Any = None) -> Optional[Dict[str, str]]:
        ...
        try:
            ...
            if not all([correct_answer, incorrect_answer]):
                return None
...
class TruthfulQAGenExtractor(BenchmarkExtractor):
    """Extractor for TruthfulQA Gen benchmarks."""

    def extract_qa_pair(self, doc: Dict[str, Any], task_data: Any = None) -> Optional[Dict[str, str]]:
        """
        TruthfulQA Gen format:
        - doc['question']: the question
        - doc['best_answer']: correct answer
        - doc['incorrect_answers']: list with incorrect answers
        """

        try:
            question = doc.get("question", "")
            correct_answer = doc.get("best_answer", "")
    ...
    def extract_contrastive_pair(self, doc: Dict[str, Any], task_data: Any = None) -> Optional[Dict[str, str]]:
        """Extract contrastive pair for TruthfulQA Gen."""
        ...
        try:
            ...
            if not all([question, correct_answer, incorrect_answer]):
                return None
...
class WiCExtractor(BenchmarkExtractor):
    """Extractor for WiC benchmark."""
    ...
    def extract_qa_pair(self, doc: Dict[str, Any], task_data: Any = None) -> Optional[Dict[str, str]]:
        ...
        - doc["word"]: word present in both sentences
        - doc["label"]: 1 - word has the same meaning in sentences, 0 - word has different meaning in sentences
        """
        try:
            ...
            if not all([sentence1, sentence2, word, label is not None]):
                return None
            
            question = f"{sentence1}\n{sentence2}\nQuestion: Is the word '{word}' used in the same way in two sentences above?"
            ...
            logger.debug(f"Error extracting WiC QA pair: {e}")
            return None

    def extract_contrastive_pair(self, doc: Dict[str, Any], task_data: Any = None) -> Optional[Dict[str, str]]:
        """Extract contrastive pair for WiC."""
        try:
            qa_pair = self.extract_qa_pair(doc, task_data)
            if not qa_pair:
                return None
            ...
            logger.debug(f"Error extracting WiC contrastive pair: {e}")
            return None
...
class MutualExtractor(BenchmarkExtractor):
    """Extractor for Mutual benchmark."""

    def extract_qa_pair(self, doc: Dict[str, Any], task_data: Any = None) -> Optional[Dict[str, str]]:
        """
        Mutual format:
        - doc["article"]: dialogue
        - doc["options']: list of possible continuations
        - doc["answers']: correct continuation
        """

        try:
            
            dialogue = doc.get("article", "")
            options = doc.get("options", [])
            answer = doc.get("answers")

            answer_idx = ord(answer) - ord("A")
            correct_answer = options[answer_idx]

            if not all([dialogue, options, answer]):
                return None
            
            return {"question": dialogue, "formatted_question": dialogue, "correct_answer": correct_answer}

        except Exception as e:
            logger.debug(f"Error extracting Mutual QA pair: {e}")
            return None

    def extract_contrastive_pair(self, doc: Dict[str, Any], task_data: Any = None) -> Optional[Dict[str, str]]:
        """Extract contrastive pair for Mutual."""
        try:
            qa_pair = self.extract_qa_pair(doc, task_data)
            if not qa_pair:
                return None
            ...
            answer = doc.get("answers")
            answer_idx = ord(answer) - ord("A")
            incorrect_answer = options[(answer_idx+1)%3]
            ...
            logger.debug(f"Error extracting Mutual contrastive pair: {e}")
            return None  

        
class MedQAExtractor(BenchmarkExtractor):
    """Extractor for MedQA benchmark."""

    def extract_qa_pair(self, doc: Dict[str, Any], task_data: Any = None) -> Optional[Dict[str, str]]:
        """
        MedQa format:
        - doc["sent1"]: question
        - doc["ending0"] doc["ending1"], doc["ending2"], doc["ending3"] answers
        - doc["lable"]: index of correct_answer (0,1,2,3)
        """

        try:

            question = doc.get("sent1", "")
            answers = [doc.get("ending0", ""), doc.get("ending1", ""), doc.get("ending2", ""), doc.get("ending3", "")]
            label = doc.get("label", 0)

            if not all([question, answers, label]):
                return None
            ...
            logger.debug(f"Error extracting MedQA QA pair: {e}")
            return None
    ...
    def extract_contrastive_pair(self, doc: Dict[str, Any], task_data: Any = None) -> Optional[Dict[str, str]]:
        ...
        try:
            ...
            logger.debug(f"Error extracting MedQA contrastive pair: {e}")
            return None   
        
class PubMedQAExtractor(BenchmarkExtractor):
    """Extractor for PubMedQA benchmark."""

    def extract_qa_pair(self, doc: Dict[str, Any], task_data: Any = None) -> Optional[Dict[str, str]]:
        """
        PubMedQa format:
        - doc[CONTEXTS]: list of sentences to concatenate to form context
        - doc[QUESTION]: question
        - doc[final_decision]: yes/no
        """
        ...
        try:
            ...
            question = doc.get("QUESTION", "")
            correct_answer = doc.get("final_decision", "")

            formatted_contexts = " ".join(s.strip() for s in contexts if isinstance(s, str) and s.strip())

            if not all([contexts, question, correct_answer]):
                return None
            ...
            if hasattr(task_data, "doc_to_text"):
                ...
                formatted_question = f"Context: {formatted_contexts} Question: {question}"

            return {"question": question, "formatted_question": formatted_question, "correct_answer": correct_answer}


        except Exception as e:
            logger.debug(f"Error extracting PubMedQA QA pair: {e}")
            return None
    ...
    def extract_contrastive_pair(self, doc: Dict[str, Any], task_data: Any = None) -> Optional[Dict[str, str]]:
        ...
        try:
            ...
            incorrect_choice = "no" if correct_choice == "yes" else "yes"

            if not all ([question, correct_choice, incorrect_choice]):
                return None
            ...
            logger.debug(f"Error extracting PubMedQA contrastive pair: {e}")
            return None
        
class HeadQAExtractor(BenchmarkExtractor):
    """Extractor for HeadQA benchmark."""

    def extract_qa_pair(self, doc: Dict[str, Any], task_data: Any = None) -> Optional[Dict[str, str]]:
        """
        HeadQa format:
        - doc['qtext']: question
        - doc['answers']: list of dictionaries with id and answer keys
        - doc['ra']: id of correct answer (1,2,3,4,5...)
        """

        try:

            question = doc.get("qtext", "")
            answers = doc.get("answers", [])
            correct_answer_id = doc.get("ra", 1)

            answers = [answer["atext"] for answer in answers]

            correct_answer = answers[correct_answer_id-1]
            ...
            logger.debug(f"Error extracting HeadQA QA pair: {e}")
            return None

    def extract_contrastive_pair(self, doc: Dict[str, Any], task_data: Any = None) -> Optional[Dict[str, str]]:
        """Extract contrastive pair for HeadQA."""
        try:
            qa_pair = self.extract_qa_pair(doc, task_data)
            if not qa_pair:
                return None
            ...
            answers = [answer["atext"] for answer in answers]
            correct_answer = qa_pair["correct_answer"]
            correct_answer_id = doc.get("ra", 0)

            # Get first incorrect choice
            incorrect_answer = None
            for i, answer in enumerate(answers):
                if i != correct_answer_id-1:
                    incorrect_answer = answer
                    break
            ...
            logger.debug(f"Error extracting HeadQA contrastive pair: {e}")
            return None

class SciQExtractor(BenchmarkExtractor):
    """Extractor for SciQ benchmark."""
    ...
    def extract_qa_pair(self, doc: Dict[str, Any], task_data: Any = None) -> Optional[Dict[str, str]]:
        ...
        - doc[support]: context
        - doc[question]: question
        - doc[correct_answer]: correct answer
        - doc[distractor1]: incorrect answer
        """

        try:

            context = doc.get("support", "")
            question = doc.get("question", "")
            correct_answer = doc.get("correct_answer", "")

            if not all([question, correct_answer]): #do not include context, because evaluation does not have it
                return None
            ...
            if hasattr(task_data, "doc_to_text"):
                ...
                formatted_question = f"Context: {context} Question: {question}"

            return {"question": question, "formatted_question": formatted_question, "correct_answer": correct_answer}


        except Exception as e:
            logger.debug(f"Error extracting SciQ QA pair: {e}")
            return None

    def extract_contrastive_pair(self, doc: Dict[str, Any], task_data: Any = None) -> Optional[Dict[str, str]]:
        """Extract contrastive pair for SciQ."""
        try:
            ...
            question = qa_pair["formatted_question"]
            correct_choice = qa_pair["correct_answer"]
            incorrect_choice = doc.get("distractor1", "")
            ...
            logger.debug(f"Error extracting SciQ contrastive pair: {e}")
            return None

        
class SOCIAL_IQAExtractor(BenchmarkExtractor):
    """Extractor for Social IQA benchmark"""
    ...
    def extract_qa_pair(self, doc: Dict[str, Any], task_data: Any = None) -> Optional[Dict[str, str]]:
        ...
        - doc['context']: context
        - doc['question']: question
        - doc['answerA'], doc['answerB'], doc['answerC']: possible answers
        - doc['label']: integer indicating correct answer (1,2,3)
        """
        ...
        try:
            ...
            if not all([context, question, choices]) or label > len(choices):
                return None
            ...
            # Get correct answer
            correct_answer = choices[label-1]

            return {"question": question, "formatted_question": formatted_question, "correct_answer": correct_answer}

        except Exception as e:
            logger.debug(f"Error extracting Social IQA pair: {e}")
            return None

    def extract_contrastive_pair(self, doc: Dict[str, Any], task_data: Any = None) -> Optional[Dict[str, str]]:
        """Extract contrastive pair for Social IQA."""
        try:
            qa_pair = self.extract_qa_pair(doc, task_data)
            if not qa_pair:
                return None
            ...
            correct_choice = choices[label-1]
            # Get first incorrect choice
            incorrect_choice = None
            for i, choice in enumerate(choices):
                if i != label-1:
            ...
            logger.debug(f"Error extracting Social IQA contrastive pair: {e}")
            return None

        
class ProstExtractor(BenchmarkExtractor):
    """Extractor for Prost benchmark"""

    def extract_qa_pair(self, doc: Dict[str, Any], task_data: Any = None) -> Optional[Dict[str, str]]:
        """
        Prost format:
        - doc['context']: context
        - doc['ex_question']: question
        - doc['A'], doc['B'], doc['C'], doc['D"]: possible answers
        - doc['label']: integer indicating correct answer
        """
        try:
            
            context = doc.get("context", "")
            question = doc.get("ex_question", "")
            choices = [doc.get("A", ""), doc.get("B", ""), doc.get("C", ""), doc.get("D", "")]
            label = doc.get("label", 0)
...
class CBExtractor(BenchmarkExtractor):
    """Extractor for CB benchmark."""

    def extract_qa_pair(self, doc: Dict[str, Any], task_data: Any = None) -> Optional[Dict[str, str]]:
        """
        CB format:
        - doc['premise']: premise
        - doc['hyphotesis'], hyphotesis
        - doc['label']: 0 - true, 1 - false, 2 - neither
        """
        try:
            premise = doc.get("premise", "")
            hyphotesis = doc.get("hypothesis", "")
            label = doc.get("label", 0)

            if not all([premise, hyphotesis]) or label not in [0, 1, 2]:
                return None
            ...
            if hasattr(task_data, "doc_to_text"):
                ...
                formatted_question = f"{premise}\nQuestion: {hyphotesis} True, False, or Neither?\nAnswer: "
            
            labels = ("True", "False", "Neither")
            correct_answer = labels[label]  
            ...
            logger.debug(f"Error extracting CB QA pair: {e}")
            return None

    def extract_contrastive_pair(self, doc: Dict[str, Any], task_data: Any = None) -> Optional[Dict[str, str]]:
        """Extract contrastive pair for CB."""
        try:
            qa_pair = self.extract_qa_pair(doc, task_data)
            if not qa_pair:
                return None
            ...
            labels = ("True", "False", "Neither")
            incorrect_label = (label + 1) % 3
            incorrect_answer = labels[incorrect_label]
            ...
            logger.debug(f"Error extracting CB contrastive pair: {e}")
            return None
        
class WSCExtractor(BenchmarkExtractor):
    """Extractor for WSC benchmark."""

    def extract_qa_pair(self, doc: Dict[str, Any], task_data: Any = None) -> Optional[Dict[str, str]]:

        """
        - doc['text']: context
        - doc['span1_text] first word
        - doc['span2_text] second word
        - doc['label'] 0 = , 1 =
        """
        ...
        try:
            ...
            if not all([text, span1_text, span2_text]) or label not in [0, 1]:
                return None
            ...
            if hasattr(task_data, "doc_to_text"):
                ...
                formatted_question = f"{text}\n In the passage above, does the pronoun **{span1_text}** refer to **{span2_text}**?"
    ...
    def extract_contrastive_pair(self, doc: Dict[str, Any], task_data: Any = None) -> Optional[Dict[str, str]]:
        ...
        try:
            ...
            correct_choice = choice1 if label == 1 else choice2
            incorrect_choice = choice1 if label == 0 else choice2
...
class RACEExtractor(BenchmarkExtractor):
    ...
    def extract_qa_pair(self, doc: Dict[str, Any], task_data: Any = None) -> Optional[Dict[str, str]]:
        ...
        doc['problems']: questions, answers, options but in a string format instead of dictionary, we have to parse it 
        """
        ...
        try:
            ...
            problem = problems[0] #take first question, options and answer
...
class QA4MREExtractor(BenchmarkExtractor):
    ...
    def extract_contrastive_pair(self, doc: Dict[str, Any], task_data: Any = None) -> Optional[Dict[str, str]]:
        ...
        try:
            ...
            correct_choice = options[answer-1]
            for i, option in enumerate(options):
                if i != answer-1:
                    incorrect_choice = option
                    break
...
class MULTIRCExtractor(BenchmarkExtractor):
    """Extractor for MULTIRC benchmark."""

    def extract_qa_pair(self, doc: Dict[str, Any], task_data: Any = None) -> Optional[Dict[str, str]]:
        """
        MULTIRC format:
        - doc['paragraph']: the paragraph
        - doc['question']: the question
        - doc['answer']: the answer
        - doc['label']: 0 - asnwer is incorrect, 1 - answer is correct
        """
        try:
            ...
            # Format the question
            if hasattr(task_data, "doc_to_text"):
                formatted_question = task_data.doc_to_text(doc) #lm-eval leaves blank for answer
                formatted_question_with_answer = formatted_question + answer #add answer to question and use label as answer
            else: #implement it without doc_to_text
                formatted_question_with_answer = f"{paragraph} \nQuestion: {question}\nAnswer: {answer}"
            
            # Get correct answer
            correct_answer = label

            # Change correct_answer to string
            correct_answer = str(correct_answer)

            return {"question": question, "formatted_question": formatted_question_with_answer, "correct_answer": correct_answer}
        
        except Exception as e:
            logger.debug(f"Error extracting MULTIRC pair: {e}")
            return None
        
    def extract_contrastive_pair(self, doc: Dict[str, Any], task_data: Any = None) -> Optional[Dict[str, str]]:
        """Extract contrastive pair for MULTIRC."""
        try:
            qa_pair = self.extract_qa_pair(doc, task_data)
            if not qa_pair:
                return None
            ...
            return {
                ...
                "incorrect_answer": str(incorrect_choice),
            }

        except Exception as e:
            logger.debug(f"Error extracting MULTIRC contrastive pair: {e}")
            return None
...
class RTEExtractor(BenchmarkExtractor):
    ...
    def extract_qa_pair(self, doc: Dict[str, Any], task_data: Any = None) -> Optional[Dict[str, str]]:
        ...
        - doc['label']: 1 if not entailment, 0 if entailment
        """
        try:
            sentence1 = doc.get("sentence1", "")
            sentence2 = doc.get("sentence2", "")
            ...
            # Get correct answer
            correct_answer = "True" if label == 0 else "False"
    ...
    def extract_contrastive_pair(self, doc: Dict[str, Any], task_data: Any = None) -> Optional[Dict[str, str]]:
        ...
        try:
            ...
            correct_choice = "True" if label == 0 else "False"
            incorrect_choice = "False" if label == 0 else "True"
...
class LogiQAExtractor(BenchmarkExtractor):
    ...
    def extract_qa_pair(self, doc: Dict[str, Any], task_data: Any = None) -> Optional[Dict[str, str]]:
        ...
        - doc['context']: context
        - doc['question']: question
        - doc['options']: list of possible answers
        - doc[label]: label of correct answer (a,b,c,d...)
        """
        ...
        try:
            ...
            question = doc.get("question", "")
            options = doc.get("options", [])
            label = doc.get("label", "a")

            label = ord(label) - ord('a')


            if not all([context, question, options]) or label >= len(options):
                return None
...
class LogiQA2Extractor(BenchmarkExtractor):
    """Extractor for LogiQA2 (Logical Reasoning) benchmark."""
    ...
    def extract_qa_pair(self, doc: Dict[str, Any], task_data: Any = None) -> Optional[Dict[str, str]]:
        ...
        - doc['question']: question
        - doc['options']: list of possible answers
        - doc[answer]: label of correct answer (0,1,2,3...)
        """
        ...
        try:
            ...
            if not all([text, question, options]) or answer >= len(options):
                return None
            ...
            if hasattr(task_data, "doc_to_text"):
                ...
                formatted_question = f"Context: {text}\nQuestion: {question}:"
            ...
            logger.debug(f"Error extracting LogiQA 2 QA pair: {e}")
            return None
    ...
    def extract_contrastive_pair(self, doc: Dict[str, Any], task_data: Any = None) -> Optional[Dict[str, str]]:
        ...
        try:
            ...
            logger.debug(f"Error extracting LogiQA 2 contrastive pair: {e}")
            return None
...
EXTRACTORS = {
    ...
    "truthfulqa_gen": TruthfulQAGenExtractor,
    ...
    "cb": CBExtractor,  # Similar format to COPA
    ...
    "logiqa2": LogiQA2Extractor,  # LogiQA 2, different format than logiqa
    ...
    "mc-taco": MCTacoExtractor,  # MC-TACO temporal reasoning
    "multirc": MULTIRCExtractor,  # Paragraph + Question + Answer, True or False format
    "mutual": MutualExtractor,  # Multiple choice format
    "naturalqs": SQuAD2Extractor,  # QA format
    "prost": ProstExtractor,  # Multiple choice format
    "pubmedqa": PubMedQAExtractor,  # yes/no
    "quac": QuACExtractor,  # Conversational QA format
    "record": SQuAD2Extractor,  # QA format
    "sciq": SciQExtractor,  # Multiple choice format
    "swag": SwagExtractor,  # Similar format to HellaSwag
    "toxigen": BoolQExtractor,  # Binary classification
    "triviaqa": SQuAD2Extractor,  # QA format
    "webqs": SQuAD2Extractor,  # QA format
    "wic": WiCExtractor,  # Binary classification
    "wsc": WSCExtractor,  # Similar format to COPA
    ...
    "arithmetic_2dm": ArithmeticExtractor, # Arithmetic tasks
    "arithmetic_2ds": ArithmeticExtractor, # Arithmetic tasks
    "arithmetic_3da": ArithmeticExtractor, #Arithmetic tasks
    "arithmetic_3ds": ArithmeticExtractor, #Arithmetic tasks
    "arithmetic_4da": ArithmeticExtractor, #Arithmetic tasks
    "arithmetic_4ds": ArithmeticExtractor, #Arithmetic tasks
    "arithmetic_5da": ArithmeticExtractor, #Arithmetic tasks
    "arithmetic_5ds": ArithmeticExtractor, #Arithmetic tasks
    ...
    "headqa_en": HeadQAExtractor,  # Spanish healthcare QA (multiple choice)
