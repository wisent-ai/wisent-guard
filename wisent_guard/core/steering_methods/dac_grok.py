...
class PropertyVector:
    ...
    name: str
    vector: torch.Tensor
    layer_index: Optional[int]  # Allow None for all layers
...
class DAC(SteeringMethod):
    ...
    def train_property(
        self, property_name: str, contrastive_pair_set: ContrastivePairSet, layer_index: Optional[int]
    ) -> Dict[str, Any]:
        """
        ...
        Args:
            ...
            layer_index: Layer index where steering will be applied (None for all layers)
        ...
        self.property_vectors[property_name] = PropertyVector(
            ...
            layer_index=layer_index,  # Can be None
    ...
    def train_multi_property(
        self, property_pairs: Dict[str, Tuple[ContrastivePairSet, Optional[int]]]
    ...
    def _compute_dynamic_alphas_online(
        ...
        for prop_name in active_properties:
            ...
            if layer_idx is not None:
                ...
                # All layers
                if hasattr(model_obj, "model") and hasattr(model_obj.model, "layers"):
                    for l in range(len(model_obj.model.layers)):
                        handle = model_obj.model.layers[l].register_forward_hook(temp_hook_fn)
                        handles.append(handle)
                elif hasattr(model_obj, "transformer") and hasattr(model_obj.transformer, "h"):
                    for l in range(len(model_obj.transformer.h)):
                        handle = model_obj.transformer.h[l].register_forward_hook(temp_hook_fn)
                        handles.append(handle)
                else:
                    logger.warning(f"Cannot find layer modules for all-layers steering for {prop_name}")
                    alpha = 0.0
                    alphas[prop_name] = alpha
                    continue
    ...
    def generate_with_dynamic_steering(
        ...
        # Set up hooks for each property at their respective layers (or all layers if layer_index is None)
        hooks = []
        ...
        def create_hook(property_name):
            ...
            def hook_fn(module, input, output):
                logger.debug(f"Hook called for {property_name} at layer {module}")
                if isinstance(output, tuple):
                    hidden_states = output[0]
                else:
                    hidden_states = output
        ...
        for prop_name in active_properties:
            if prop_name in self.property_vectors:
                ...
                if layer_idx is not None:
                    ...
                    hooks.append(handle)
                    logger.debug(f"Hook registered for layer {layer_idx}")
                else:
                    # All layers
                    for l in range(len(layers)):
                        ...
                        handle = layer_module.register_forward_hook(hook_fn)
                        hooks.append(handle)
                        logger.debug(f"Hook registered for layer {l}")
...
# NOTE: Updated to handle layer_index=None by registering hooks on all transformer layers for the property,
# applying the same steering vector to hidden states in every layer. This matches the behavior of the original
# DAC implementation that works across all layers. The steering vector is computed as before (assuming activations
# were extracted appropriately), and applied uniformly to all layers when layer_index is None.
