...
class WisentQwen2Config(Qwen2Config):
    ...
    def __init__(
        ...
        caa_layer_id: int = 23,
        caa_alpha: float = 0.45,
        steering_vector_path: str = "./vectors/hendrycks_math/steering_vector.safetensors",
...
class WisentQwen2ForCausalLM(Qwen2ForCausalLM):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *model_args, **kwargs):
        ...
        if not has_weights and local_path.exists() and (local_path / "config.json").exists():
            ...
            # Now load config from local path
            from transformers import AutoConfig

            config = AutoConfig.from_pretrained(pretrained_model_name_or_path)

            print(f"Loading weights from base model: {base_model_name}")
