"""
Qwen2.5-Coder-7B Coding Task Optimization with Optuna

This script demonstrates a coding-focused optimization pipeline for Qwen2.5-Coder-7B
model using MBPP datasets with Optuna hyperparameter optimization,
...
USAGE:
    ...
    HF_ALLOW_CODE_EVAL="1" python qwen25_coder_optuna_coding.py

    # Custom model path and batch size
    HF_ALLOW_CODE_EVAL="1" python qwen25_coder_optuna_coding.py --model-path Qwen/Qwen2.5-Coder-7B-Instruct --batch-size 2

    # Enable WandB logging
    HF_ALLOW_CODE_EVAL="1" python qwen25_coder_optuna_coding.py --use-wandb --wandb-project qwen-coding-optimization

    # Quick test run
    HF_ALLOW_CODE_EVAL="1" python qwen25_coder_optuna_coding.py --n-trials 10 --train-limit 50 --val-limit 25

EXPECTED RESULTS:
- Baseline Qwen2.5-Coder-7B typically achieves ~60-80% on MBPP (excellent for coding)
...
- Results: outputs/qwen25_coder_optimization/
- WandB logs: https://wandb.ai/your-project/qwen-coding-optimization
...
def get_recommended_config_for_qwen25_coder() -> Dict[str, Any]:
    """Get recommended configuration values for Qwen2.5-Coder-7B coding optimization."""
    return {
        ...
        "train_limit": 100,  # Good balance for optimization
...
def create_qwen25_coder_config(args) -> OptimizationConfig:
    """Create optimized configuration for Qwen2.5-Coder-7B coding optimization."""
...
class Qwen25CoderPipeline(OptimizationPipeline):
    """Specialized pipeline for Qwen2.5-Coder-7B coding optimization with MBPP datasets."""
    ...
    def _objective_function(self, trial) -> float:
        ...
        try:
            self.logger.info(f"🔬 Trial {trial.number}: Starting coding task optimization")
            ...
            self.logger.info(f"🎯 Trial {trial.number}: Validation coding accuracy = {validation_accuracy:.4f}")
    ...
    def _log_enhanced_results(self, study, final_results):
        """Log enhanced results with coding-specific analysis."""
        self.logger.info("=" * 80)
        self.logger.info("🤖 QWEN2.5-CODER-7B CODING OPTIMIZATION RESULTS")
        self.logger.info("=" * 80)
        ...
        self.logger.info("📈 Test Results on MBPP:")
        ...
        # Performance context
        if baseline_acc > 0.6:
            self.logger.info("🎉 Excellent baseline performance! Qwen2.5-Coder excels at coding tasks")
        elif baseline_acc > 0.4:
            self.logger.info("👍 Good baseline performance for coding tasks")
        else:
...
def main():
    """Main entry point for Qwen2.5-Coder-7B coding optimization."""
    parser = argparse.ArgumentParser(
        description="Qwen2.5-Coder-7B Coding Task Optimization with MBPP",
    ...
    parser.add_argument(
        "--train-limit", type=int, default=None, help="Number of training samples to load (default: 100)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--val-limit", type=int, default=None, help="Number of validation samples to load (default: 50)"
    )
    parser.add_argument("--test-limit", type=int, default=None, help="Number of test samples to load (default: 100)")

    # Optimization configuration
    parser.add_argument(
        "--study-name", type=str, default=None, help="Optuna study name (default: qwen25_coder_optimization)"
    )
    ...
    # Utility options
    parser.add_argument(
        "--quick-test", action="store_true", help="Quick test run (10 trials, 20 train, 10 val samples)"
    )
    ...
    logger.info("🤖 QWEN2.5-CODER-7B CODING OPTIMIZATION")
    logger.info("=" * 80)
    ...
    if os.environ.get("HF_ALLOW_CODE_EVAL") != "1":
        logger.warning("⚠️  HF_ALLOW_CODE_EVAL not set. MBPP requires code execution.")
    ...
    try:
        ...
        pipeline = Qwen25CoderPipeline(config)

        # Run optimization
        logger.info("🚀 Starting coding optimization for Qwen2.5-Coder-7B...")
        results = pipeline.run_optimization()

        # Enhanced result display
        pipeline._log_enhanced_results(pipeline._create_optuna_study(), results)

        logger.info("✅ Qwen2.5-Coder-7B coding optimization completed successfully!")
