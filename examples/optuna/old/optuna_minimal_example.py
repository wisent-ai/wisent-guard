...
class MinimalSteeringPipeline(OptimizationPipeline):
    ...
    def _objective_function(self, trial):
        ...
        try:
            ...
            self.logger.info(
                f"Trial {trial.number}: Testing {steering_method} with α={hyperparams['steering_alpha']:.3f}"
            )
            ...
            self.logger.info(
                f"Trial {trial.number}: {steering_method} α={hyperparams['steering_alpha']:.3f} → accuracy={validation_accuracy:.3f}"
            )
