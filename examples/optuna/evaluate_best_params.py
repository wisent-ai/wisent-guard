...
def evaluate_from_study(args) -> dict[str, Any]:
    ...
    pipeline, study = OptimizationPipeline.from_saved_study(
        ...
        } if any([args.model_name, args.test_dataset, args.test_limit, args.device, args.batch_size]) else None
    )
    ...
    if args.cross_dataset:
        ...
        for dataset in datasets:
            ...
            try:
                result = pipeline.evaluate_on_dataset(
                    study.best_params, 
                    dataset, 
                    args.test_limit
                )
...
def main():
    ...
    # Mode selection
    parser.add_argument("--manual-eval", action="store_true",
                       help="Use manually specified parameters instead of loading from study")
    
    # Study loading options
    parser.add_argument("--study-db", type=str,
                       help="Path to Optuna study database (.db file)")
    parser.add_argument("--config", type=str,
                       help="Path to saved configuration JSON")
    
    # Evaluation options
    parser.add_argument("--test-dataset", type=str, default="gsm8k",
    ...
    parser.add_argument("--cross-dataset", action="store_true",
                       help="Evaluate on multiple datasets (gsm8k, hendrycks_math, aime)")
    ...
    parser.add_argument("--layer-id", type=int, default=15,
                       help="Layer ID for manual evaluation")
    parser.add_argument("--steering-method", type=str, default="caa", choices=["caa", "dac"],
                       help="Steering method for manual evaluation")
    parser.add_argument("--steering-alpha", type=float, default=0.5,
                       help="Steering alpha for manual evaluation")
    parser.add_argument("--entropy-threshold", type=float, default=1.5,
                       help="DAC entropy threshold (manual mode)")
    parser.add_argument("--ptop", type=float, default=0.5,
                       help="DAC ptop parameter (manual mode)")
    parser.add_argument("--max-alpha", type=float, default=2.0,
                       help="DAC max alpha parameter (manual mode)")
