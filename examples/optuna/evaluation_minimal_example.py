...
def evaluate_single_params(params: dict[str, Any], config_name: str = "default") -> dict[str, Any]:
    ...
    try:
        ...
        traceback.print_exc()
        return {"config_name": config_name, "params": params, "error": str(e)}
    finally:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Evaluation minimal example", formatter_class=argparse.RawDescriptionHelpFormatter, epilog=__doc__
    )

    parser.add_argument("--compare-params", action="store_true", help="Compare multiple parameter sets")
    parser.add_argument("--cross-dataset", action="store_true", help="Evaluate across multiple datasets")
    parser.add_argument("--study-db", type=str, help="Load parameters from existing study database")
