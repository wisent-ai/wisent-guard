...
class EfficientBenchmarkEvaluator:
    ...
    def __init__(self, config: EfficientEvaluationConfig):
        ...
        self.wandb_run = None
        self.logger = logger
        self.results = {"config": asdict(config), "timestamp": datetime.now().isoformat()}
    ...
    def _load_model_once(self):
        ...
        self.model = AutoModelForCausalLM.from_pretrained(
            model_name, torch_dtype=torch.float16, device_map={"": 0}, low_cpu_mem_usage=True, local_files_only=True
        )
    ...
    def _prepare_dataset_activations_with_samples(
    ...
    def _log_detailed_predictions_to_wandb(
        self, test_samples: List[Dict], y_true: np.ndarray, y_pred: np.ndarray, y_pred_proba: np.ndarray, layer: int
    ):
        ...
        self.wandb_run.log(
            {
                ...
                "test_accuracy_breakdown": {
                    ...
                    "accuracy": sum(row["correct_prediction"] for row in prediction_data) / len(prediction_data)
                    if prediction_data
                    else 0,
                },
            }
    ...
    def run_efficient_three_phase_pipeline(self) -> Dict[str, Any]:
        ...
        self.logger.info(
            f"Hyperparameter search: {len(self.config.layers_to_search)} layers × {len(self.config.c_values)} C values"
        )
        ...
        train_acts, train_samples = self._prepare_dataset_activations_with_samples(
            self.config.train_benchmark, self.config.train_limit
        )
        val_acts, val_samples = self._prepare_dataset_activations_with_samples(
            self.config.val_benchmark, self.config.val_limit
        )
        test_acts, test_samples = self._prepare_dataset_activations_with_samples(
            self.config.test_benchmark, self.config.test_limit
        )
        ...
        if best_config:
            ...
            if self.config.enable_wandb and self.wandb_run:
                self._log_detailed_predictions_to_wandb(
                    test_samples, y_test, test_pred, test_pred_proba, best_config["layer"]
                )

            # Save results
            self.results.update(
                {
                    "hyperparameter_search": [{k: v for k, v in r.items() if k != "classifier"} for r in all_results],
...
def main():
    ...
    # Model configuration
    parser.add_argument("--model", default="/workspace/models/llama31-8b-instruct-hf", help="Model name or path")
    parser.add_argument(
        "--layers",
        nargs="+",
        type=int,
    ...
    # Hyperparameter search
    parser.add_argument(
        "--c-values", nargs="+", type=float, default=[0.1, 1.0, 10.0], help="C values for LogisticRegression"
    )

    # Output configuration
    parser.add_argument("--output-dir", default="efficient_evaluation_results", help="Output directory")
    parser.add_argument("--experiment-name", default="efficient_wisent_guard_evaluation", help="Experiment name")
