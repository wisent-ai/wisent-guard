"""
This is a copy of CAA wisent_guard/core/steering_methods/caa.py created for test development.
It applies steering for all tokens, whereas our caa implementation apply (currently) only for -2 token.
"""
...
from wisent_guard.core.aggregation import ControlVectorAggregationMethod, create_control_vector_from_contrastive_pairs
from wisent_guard.core.contrastive_pairs import ContrastivePairSet
from wisent_guard.core.normalization import VectorNormalizationMethod, VectorNormalizer
...
class CAA(SteeringMethod):
    ...
    def apply_steering(
        ...
        if len(activations.shape) == 3:  # [batch, seq, hidden]
            # Apply to ALL tokens (CAA reference behavior when instruction detection fails)
