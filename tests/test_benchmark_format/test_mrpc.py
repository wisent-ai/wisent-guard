...
# Load MRPC task
tasks = get_task_dict(["mrpc"])
mrpc_task = tasks["mrpc"]
...
try:
    docs = list(mrpc_task.training_docs())
    if docs:
        split_used = "train"
except:
    pass
...
if not docs:
    try:
        docs = list(mrpc_task.validation_docs())
        if docs:
...
if not docs:
    ...
    print(f"Available methods: {[method for method in dir(mrpc_task) if 'docs' in method]}")
    exit(1)

print(f"MRPC dataset loaded: {len(docs)} samples from {split_used} split")
print("\n" + "="*80)
print("EXAMPLES OF MRPC DATA:")
print("="*80)
