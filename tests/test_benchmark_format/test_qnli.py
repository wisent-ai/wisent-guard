...
# Load QNLI task
tasks = get_task_dict(["qnli"])
qnli_task = tasks["qnli"]
...
try:
    docs = list(qnli_task.training_docs())
    if docs:
        split_used = "train"
except:
    pass
...
if not docs:
    try:
        docs = list(qnli_task.validation_docs())
        if docs:
...
if not docs:
    ...
    print(f"Available methods: {[method for method in dir(qnli_task) if 'docs' in method]}")
    exit(1)

print(f"QNLI dataset loaded: {len(docs)} samples from {split_used} split")
print("\n" + "="*80)
print("EXAMPLES OF QNLI DATA:")
print("="*80)
