...
# Load DROP task
tasks = get_task_dict(["drop"])
drop_task = tasks["drop"]
...
try:
    docs = list(drop_task.training_docs())
    if docs:
        split_used = "train"
except:
    pass
...
if not docs:
    try:
        docs = list(drop_task.validation_docs())
        if docs:
...
if not docs:
    ...
    print(f"Available methods: {[method for method in dir(drop_task) if 'docs' in method]}")
    exit(1)

print(f"DROP dataset loaded: {len(docs)} samples from {split_used} split")
print("\n" + "="*80)
print("EXAMPLES OF DROP DATA:")
print("="*80)
