...
try:
    docs = list(qqp_task.training_docs())
    if docs:
        split_used = "train"
except:
    pass
...
if not docs:
    try:
        docs = list(qqp_task.validation_docs())
        if docs:
...
if not docs:
    ...
    print(f"Available methods: {[method for method in dir(qqp_task) if 'docs' in method]}")
    exit(1)

print(f"QQP dataset loaded: {len(docs)} samples from {split_used} split")
print("\n" + "="*80)
print("EXAMPLES OF QQP DATA:")
print("="*80)
