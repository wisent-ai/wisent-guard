...
# Load SST2 task
tasks = get_task_dict(["sst2"])
sst2_task = tasks["sst2"]
...
try:
    docs = list(sst2_task.training_docs())
    if docs:
        split_used = "train"
except:
    pass
...
if not docs:
    try:
        docs = list(sst2_task.validation_docs())
        if docs:
...
if not docs:
    ...
    print(f"Available methods: {[method for method in dir(sst2_task) if 'docs' in method]}")
    exit(1)

print(f"SST2 dataset loaded: {len(docs)} samples from {split_used} split")
print("\n" + "="*80)
print("EXAMPLES OF SST2 DATA:")
print("="*80)
