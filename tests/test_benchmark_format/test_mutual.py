...
# Load Mutual task
tasks = get_task_dict(["mutual"])
mutual_task = tasks["mutual"]
...
try:
    docs = list(mutual_task.training_docs())
    if docs:
        split_used = "train"
except:
    pass
...
if not docs:
    try:
        docs = list(mutual_task.test_docs())
        if docs:
            split_used = "test"
    except:
        pass
...
if not docs:
    try:
        docs = list(mutual_task.validation_docs())
        if docs:
...
if not docs:
    ...
    print(f"Available methods: {[method for method in dir(mutual_task) if 'docs' in method]}")
    exit(1)

print(f"Mutual dataset loaded: {len(docs)} samples from {split_used} split")
print("\n" + "="*80)
print("EXAMPLES OF MUTUAL DATA:")
print("="*80)
