...
# Load ReCoRD task
tasks = get_task_dict(["record"])
record_task = tasks["record"]
...
try:
    docs = list(record_task.training_docs())
    if docs:
        split_used = "train"
except:
    pass
...
if not docs:
    try:
        docs = list(record_task.test_docs())
        if docs:
            split_used = "test"
    except:
        pass
...
if not docs:
    try:
        docs = list(record_task.validation_docs())
        if docs:
...
if not docs:
    ...
    print(f"Available methods: {[method for method in dir(record_task) if 'docs' in method]}")
    exit(1)

print(f"ReCoRD dataset loaded: {len(docs)} samples from {split_used} split")
print("\n" + "="*80)
print("EXAMPLES OF RECORD DATA:")
print("="*80)
