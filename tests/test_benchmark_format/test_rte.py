...
try:
    docs = list(rte_task.training_docs())
    if docs:
        split_used = "train"
except:
    pass
...
if not docs:
    try:
        docs = list(rte_task.validation_docs())
        if docs:
...
if not docs:
    ...
    print(f"Available methods: {[method for method in dir(rte_task) if 'docs' in method]}")
    exit(1)

print(f"RTE dataset loaded: {len(docs)} samples from {split_used} split")
print("\n" + "="*80)
print("EXAMPLES OF RTE DATA:")
print("="*80)
