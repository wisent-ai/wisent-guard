...
def run_training(model, model_name, layer, scale, normalize):
    ...
    rot_data.use("custom")
    absolute_path = "./tests/untruthfulness/question_answers.json"
    ...
    save_dir = f"./tests/truthfulness/steering_output_{model_name}/steering_output_layer{layer}_scale{abs(scale)}_{normalize_str}"
...
for model_path, cfg in configs.items():
    ...
    output_file = f"./tests/untruthfulness/grid_search_results_{cfg['model_name']}.json"
