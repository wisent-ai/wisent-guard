...
def run_training(model, model_name, data_limit, layer, scale, normalize):
    ...
    save_dir = f"./tests/arithmetic/steering_output_{model_name}/steering_output_layer{layer}_scale{abs(scale)}_{normalize_str}"
...
for model_path, cfg in configs.items():
    ...
    output_file = f"./tests/arithmetic/grid_search_results_{cfg['model_name']}.json"
